...
@GET
@Path("/bootstrap/{deviceName}")
@Produces("application/json")
public static String bootstrap(@PathParam("deviceName") String device) throws UnknownHostException
{
	String dbs="";
	int modelNumber=2015;	
	dbs="bootstrap";
	MongoClient mc=new MongoClient("localhost",27017);
	DB db=mc.getDB("adas");
	DBObject doc=new BasicDBObject();
	DBObject squery=new BasicDBObject();
	DBCollection tab=db.getCollection(dbs);
	squery.put("Serial Number",i);
	squery.put("Model Number",modelNumber);
	squery.put("Manufacturer", "SPMS");
	squery.put("Device Type","Sensor");
	squery.put("Reboot", "No");
	squery.put("Error Code", 0);
	squery.put("Supported Binding and Modes", "U");
	i++;
	tab.insert(squery);
	return "Bootstrapped Device "+device;
}
